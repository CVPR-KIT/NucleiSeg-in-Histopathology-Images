...
from  networkModules.conv_modules_elunet import DoubleConv,DownSample,UpSample,OutConv

class ELUnet(nn.Module):
    def __init__(self, config, in_channels = 1,n:int = 8):
        """ 
        ...
        self.up_512_64 = UpSample(8*n,n,8, self.kernel_size)
        self.up_512_128 = UpSample(8*n,2*n,4, self.kernel_size)
        self.up_512_256 = UpSample(8*n,4*n,2, self.kernel_size)
        self.up_512_512 = UpSample(8*n,8*n,0, self.kernel_size)

        self.up_256_64 = UpSample(4*n,n,4, self.kernel_size)
        self.up_256_128 = UpSample(4*n,2*n,2, self.kernel_size)
        self.up_256_256 = UpSample(4*n,4*n,0, self.kernel_size)

        self.up_128_64 = UpSample(2*n,n,2, self.kernel_size)
        self.up_128_128 = UpSample(2*n,2*n,0, self.kernel_size)

        self.up_64_64 = UpSample(n,n,0, self.kernel_size)
     
        # ------ Decoder block ---------------
        self.dec_4 = DoubleConv(2*8*n,8*n, self.kernel_size)
        self.dec_3 = DoubleConv(3*4*n,4*n, self.kernel_size)
        self.dec_2 = DoubleConv(4*2*n,2*n, self.kernel_size)
        self.dec_1 = DoubleConv(5*n,n, self.kernel_size)
        # ------ Output convolution

        self.out_conv = OutConv(n,out_channels, self.kernel_size)
